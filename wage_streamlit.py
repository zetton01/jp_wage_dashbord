...
st.header('■2019年：一人あたりの平均賃金のヒートマップ')
...
df_pref_map = pd.merge(df_pref_map, jp_lat_lon, on='都道府県名') # on= キーとなる列名
# 最小値0、最大値１とする正規化処理　Y　＝　X - Xmin / Xmax -　Xmin
...
layer = pdk.Layer(
    ...
    threshold=0.3, #どこの値を閾値として表示するかを設定
...
st.text('本結果はRESAS（地域経済分析システムを加工して作成')
